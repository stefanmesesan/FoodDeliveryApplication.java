...
CREATE TABLE restaurants (id UUID PRIMARY KEY, name TEXT, address TEXT, phoneNumber TEXT, rating FLOAT, addedBy UUID REFERENCES users(id), needDeletion BOOLEAN);
...
CREATE TABLE orders (id UUID PRIMARY KEY, user_id UUID REFERENCES users(id), restaurant_id UUID REFERENCES restaurants(id), status VARCHAR, total_price FLOAT, created_at TEXT, order_placed BOOLEAN, delivery_guy_id UUID);
